...
public class Ueberpruefer {
	public static boolean registrierDatenPruefen(String[][] benutzer, String vorname, String nachname) {
		if (!onlyLetters(vorname) || !onlyLetters(nachname)) {
			return false;
		}
		if (benutzer != null) {
	...
	public static boolean onlyLetters(String zuTesten) {
		return zuTesten.matches("[a-zA-Z]+");
	}
	...
	public static boolean stornierPruefer(String[][][] wohnungen, int wohnung, String vorname, String nachname,
			String zeitraum) {
			...
			for (int i = 0; i < wohnungen[wohnung].length; i++) {
				if (wohnungen[wohnung][i][0].equals(vorname) && wohnungen[wohnung][i][1].equals(nachname)) {
					...
					try {
						...
						System.out.println(""+ anfang + "  " + c.getTime());
		...
		System.out.println(wohnungen[wohnung].length);
		return false;
	}
		...
		for (int i = 0; i < 2; i++) {
			try {
				...
				if (date.after(new Date())
						&& vonBis[i].split("\\.")[2].equals("" + Calendar.getInstance().get(Calendar.YEAR))) {
	...
	public static boolean kontrolle(String wohn[][][], String datum, int wohnung) throws ParseException { // Methode
																											// zur
																											// Kontrolle
																											// ob
																											// Wohnungen
	...
	public static boolean[] getFreieWohnungen(String wohn[][][], String datum) throws ParseException { // Methode
																										// zur
																										// Kontrolle
																										// ob
																										// Wohnungen
			...
			if (wohn != null) {
				for (int x = 0; x < wohn.length; x++) {
					for (int i = 0; i < wohn[x].length; i++) {
						...
						Date dateGV = sdf.parse(gebuchterzeitraum[0]);
						Date dateGB = sdf.parse(gebuchterzeitraum[1]);
						if (dateGB.before(dateBV) || dateGB.equals(dateBV) || dateGV.after(dateBB)
								|| dateGV.equals(dateBB)) {
